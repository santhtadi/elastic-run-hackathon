...
class YoloDetector:
    ...
    def detect(self, img, conf=0.1, nms_thresh=0.3, non_max_suppression=True):
        class_conf_dict = {k: conf for i, k in enumerate(self.classes)}
        final_result = {k: [] for k in self.classes}
        final_result_with_conf = {k: [] for k in self.classes}
        ...
        for key, index in indices.items():
            for i in index:
                ...
                final_result_with_conf[key].append([self.get_output_format(boxes[key][select]),
                                                    confidences[key][select]])
        return final_result, final_result_with_conf
